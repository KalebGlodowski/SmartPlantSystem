...
void _waterPumpOverride() {
  if (pumpCanBeOn_Override == true) {
    Serial.println("Water pump manual override pressed. Plant may not need water. Water pump turning on for 1/2 second.");
    digitalWrite(WATERPIN, HIGH);
    delay(500);
    digitalWrite(WATERPIN, LOW);
...
void _OLED_AllData() {
  _BMESensor();           //Every 5s, Temperature, Pressure, and Humidity readings from the BME
  _soilSensor();          //Every 5s, Soil Moisture level
  _dustSensor();          //Dust levels
  _AQSensor();            //Air quality value and pollution level
  _dateTime();            //Gets the current data/time to timestamp the OLED
  //Clear OLED
  oled.clearDisplay();
  oled.display();
  //OLED Settings
